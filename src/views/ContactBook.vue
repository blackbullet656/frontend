...
async function retrieveContacts(page) {
    try {
        const chunk = await contactsService.fetchContacts(page); // Correct function
        ...
        console.error(error); // Consider user notification for better UX
    }
}
...
watch(currentPage, () => retrieveContacts(currentPage.value), {
    immediate:
        true
});
...
justify-content-round align-items-center">
